...
import WeekdaySimpleFn = moment.WeekdaySimpleFn;
...
export default {
    ...
    getDateData(options) {
        ...
        for (let i = minMonth; i <= maxMonth; i++) {
            let label = (i + 1) + locale.month;
            if (options && options.needMonthInString) {
                label = monthNames[i] + locale.month;
            }
            months.push({
                value: i,
                label: label,
    ...
    getValueDataSource() {
        ...
        if (mode === LEAFLET) {
            dataSource = [...this.getDateData({needMonthInString: true})];
